...
struct fobj *fobj_rw_paged_alloc(unsigned int num_pages)
{
	...
	struct fobj_rwp *rwp = calloc(1, sizeof(*rwp));
	size_t size;
				...
				void *va)
{
	...
	struct rwp_aes_gcm_iv iv = {
		{ (vaddr_t)state, state->iv >> 32, state->iv }
	};
...
struct fobj *fobj_ro_paged_alloc(unsigned int num_pages, void *hashes,
				 void *store)
{
	struct fobj_rop *rop = calloc(1, sizeof(*rop));
...
{
	struct fobj *f = calloc(1, sizeof(*f));
...
struct fobj *fobj_sec_mem_alloc(unsigned int num_pages)
{
	...
	void *va = phys_to_virt(tee_mm_get_smem(f->mm), MEM_AREA_TA_RAM);
